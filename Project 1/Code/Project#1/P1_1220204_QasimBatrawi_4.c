...
void InsertDistrict(char districtName[50] , District *p , District *List); // we will use this method to insert districts, in this project, i have built it in the way to insert in any place. but in this project, we will use it as inser last.
...
int main(){
    ...
    while (option != 14){
        ...
        if (option < 1 || option > 14){
            ...
            else if (!chose[2] && hasDistricts(List)){ // we can add district to the unsorted districts if the list is empty.
                printf("Districts have not been sorted yet.\n\n") ;
            }
            else{
                ...
                if (district == NULL){
                    ...
                    if (FindTown(townName , district , List) != NULL){
                        ...
                        if (populationCount < 0){
                            ...
                            chose[4-1] = 0; // the towns are not sorted, since we have added a new town
                            chose[option - 1] = 1;
                        }
                    }
                }
            ...
            if (!chose[0]){
                ...
                if (district == NULL){
                ...
                else if (!hasTowns(district , List)){
                    ...
                    if (town == NULL){
                        ...
                        DeleteTown(town , district , List) ; // this will not effect on the district (if its towns are sorted or not).
...
District *copyList(District *List){
    ...
    District *district = List->NextDistrict ;
    District *tempLastDistrict = tempList ;
...
void InsertDistrict(char districtName[50] , District *p , District *List){ // insert after district p
    ...
    if (temp != NULL){ // it may be NULL because of out of memory
        ...
        if (p->NextDistrict != NULL){ // if we insert after lastDistrict, we will not enter this if statement
...
void PrintTowns(District *district , District *List){ // we must check that the district  exist first
    ...
    while (fgets(str , sizeof(str) , fp)){
        ...
        if (!isalpha(str[i])){
            ...
            if (i != lengthOfStr) { // if there is a text after the number(population).
                printf("The string %d does not match the details of program. String does not match the details of the program.\n", Line++) ;
                continue;
            }
...
void insertInfoToList(District *List , char districtName[50] , char townName[50] , char population[50]){
    ...
    if (district != NULL){
        ...
        InsertDistrict(districtName , lastDistrict , List) ; // insert last. the global lastDistrict will be the inserted district.
        InsertTown(townName , populationCount , lastDistrict->lastTown , lastDistrict , List) ;
    }
...
int getMaxDigits(District *district , District *List){ // get the maximum number of digits for the population of a town
...
District *stringRadixSortForDistricts(District *List){
    ...
    while (maxChars != 0){
        ...
        DeleteAllDistricts(List) ; // the global lastDistrict will point to the head.

        for (int i = 0 ; i < 55 ; i++){
            int sizeOfAlpha = sizeOfList(alpha[i]) ;
            Alphabetical *tempAlpha = alpha[i]->NextDistrict ;
            for (int j = 0 ; j < sizeOfAlpha ; j++){
                InsertDistrict(tempAlpha->district , lastDistrict , List) ; // insert last. the global lastDistrict will change each loop.
                lastDistrict->sortedTowns = tempAlpha->sortedTowns ;
